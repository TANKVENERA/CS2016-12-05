...
public class C_EditDist {
    ...
    String getDistanceEdinting(String one, String two) {
        ...
        int j = m;
//        while(i != 0 && j != 0){
//            int min = get_min(data_dist[i-1][j], data_dist[i][j-1], data_dist[i-1][j-1]);
//        }
    ...
    int[] get_ind(int[] mass, int i1, int j1){
        int[] res = new int[2];

        return res;
    }

    char get_action(int a, int b, int c, int n){
        char ch = ' ';
        int com = get_min(a, b, c);
        if (com == a){
           ch = '-';
        }else if (com == a){
            ch = '+';
